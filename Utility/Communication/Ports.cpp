...
//  */
// unsigned int readMessage(Port *RxPort, unsigned long long *messageContainer, unsigned long long *messageDataContainer, unsigned int messageType)
// {
//     uint8_t retVal              = PORT_E_EMPTY;

//     /* Set the last index used by the reading component */
//     uint8_t internalIndexBuffer = RxPort->bufferReadIndex;

//     for(uint8_t i = 0u; i < RxPort->bufferSize; ++i)
//     {
//         if( /* If the message buffer is not empty */
//             (RxPort->messagePresentFlag[internalIndexBuffer] == true) &&
//             (   /* No specific message type is expected */
//                 (NO_SPECIFIC_TYPE == messageType) ||
//                 /* The expected message type is found */
//                 (RxPort->messageContainer[internalIndexBuffer] == messageType)
//                 ///TODO: add flagged message type definition for broad message decoding
//             )
//           )
//         {
//             /* Copy the message type (this only has value if )*/
...
// uint16_t testPort(Port *TargetPort)
// {
//     return PORT_E_OK;
// }